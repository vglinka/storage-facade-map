...
import { MapInterface as TestedInterface, getBase, getMapStorage } from '../src/index';
...
it('Async: read/write', async () => {
  ...
  expect(getMapStorage(storage).get('value')).toEqual({ c: [40, 42] });
});
...
it('Async: delete key', async () => {
  ...
  expect(getMapStorage(storage).get('value')).toEqual(undefined);
  ...
  expect(getMapStorage(storage).get('newKey')).toEqual(undefined);
});
...
it('Async: clear storage', async () => {
  ...
  expect(getMapStorage(storage).get('value')).toEqual(4);
...
it('Async: size', async () => {
  ...
  expect(getMapStorage(storage).size).toEqual(2);
...
it('Async: key', async () => {
  ...
  expect(Array.from(getMapStorage(storage))[0][0]).toEqual('value');
